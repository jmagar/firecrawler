...
const LANGUAGE_PATTERNS = {
  ...
  es: ['es', 'spanish', 'esp', 'es-es', 'es-mx', 'es-ar', 'es-co', 'es-cl', 'es-pe', 'es-ve'], // Spanish
  ...
  uk: ['uk', 'ukrainian', 'ukr', 'ua'],                           // Ukrainian
...
export function getLanguageExcludePatterns(allowedLanguage: string): string[] {
  ...
  for (const excludedLang of Array.from(excludedLanguages)) {
    ...
    if (excludedLang.startsWith(normalizedAllowed + '-') || 
        excludedLang.startsWith(normalizedAllowed + '_')) {
      continue;
    }

    // Generate patterns for each URL indicator type
    for (const indicator of URL_LANGUAGE_INDICATORS) {
      const pattern = indicator.replace(/\{\{lang\}\}/g, escapeRegex(excludedLang));
 ...
 */
export function shouldExcludeUrl(url: string, allowedLanguage: string): boolean {
  const excludePatterns = getLanguageExcludePatterns(allowedLanguage);
  
  if (excludePatterns.length === 0) {
    return false;
  }

  // Test URL against each exclusion pattern
...
export function isLanguageSupported(languageCode: string): boolean {
  return Object.prototype.hasOwnProperty.call(LANGUAGE_PATTERNS, languageCode.toLowerCase());
}